...
async def process_payment(update, context, method):
    ...
    pedido = {
        ...
        "cliente": cadastros.get(user_id, {})
    }

    pedidos_pendentes[codigo_pedido] = pedido

    if user_id not in pedidos:
        pedidos[user_id] = []
    pedidos[user_id].append(pedido)
...
async def confirm_payment(update, context, codigo_pedido):
    ...
    if codigo_pedido not in pedidos_pendentes:
        await update.callback_query.answer("❌ Pedido não encontrado!")
        return

    pedido = pedidos_pendentes[codigo_pedido]
    ...
    pedido["link_download"] = link_download
    pedido["expira"] = expira
    ...
    del pedidos_pendentes[codigo_pedido]
...
async def show_orders(update, context):
    ...
    user_id = update.callback_query.from_user.id
    user_pedidos = pedidos.get(user_id, [])
...
async def cmd_pendentes(update, context):
    ...
    for codigo, pedido in pedidos_pendentes.items():
        cliente = pedido.get("cliente", {})
        text += f"🔹 *{codigo}* - R$ {pedido['total']:.2f}\n"
        text += f"   👤 {cliente.get('nome', 'Nao informado')}\n"
        text += f"   📦 {len(pedido['itens'])} itens\n"
...
async def cmd_aprovar(update, context):
    ...
    if codigo not in pedidos_pendentes:
        await update.message.reply_text(f"❌ Pedido `{codigo}` não encontrado!")
        return

    pedido = pedidos_pendentes[codigo]
...
async def main():
    ...
    if WEBHOOK_URL:
        ...
        while True:
            ...
            print(f"🔄 Bot ativo - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    else:
